...
describe EventMachine::APN::Notification do
  ...
  describe "#initialize" do
    it "raises an exception if the token is blank" do
      expect { EM::ApnManager::Notification.new(nil) }.to raise_error
      expect { EM::ApnManager::Notification.new("") }.to raise_error
    end

    it "raises an exception if the token is less than or greater than 32 bytes" do
      expect { EM::ApnManager::Notification.new("0" * 63) }.to raise_error
      expect { EM::ApnManager::Notification.new("0" * 65) }.to raise_error
    end
  end

  describe "#token" do
    it "returns the token" do
      notification = EM::ApnManager::Notification.new(token)
  ...
  describe "#payload" do
    ...
    it "returns custom properties as well" do
      notification = EM::ApnManager::Notification.new(token, {}, {:line => "I'm super bad"})
      payload = Yajl::Parser.parse(notification.payload)
  ...
  describe "#data" do
    it "returns the enhanced notification in the supported binary format" do
      notification = EM::ApnManager::Notification.new(token, {:alert => "Hello world"})
    ...
    it "handles UTF-8 payloads" do
      string = "✓ Please"
      notification = EM::ApnManager::Notification.new(token, {:alert => string})
      data = notification.data.unpack("cNNnH64na*")
      data[4].should == token
  ...
  describe "#validate!" do
    it "raises PayloadTooLarge error if PAYLOAD_MAX_BYTES exceeded" do
      notification = EM::ApnManager::Notification.new(token, {:alert => "X" * 512})

      lambda {
        notification.validate!
      }.should raise_error(EM::ApnManager::Notification::PayloadTooLarge)
    end
  end
  ...
  describe "#identifier=" do
    ...
    it "can be set in the initializer" do
      notification = EM::ApnManager::Notification.new(token, {}, {}, {:identifier => 12345})
      notification.identifier.should == 12345
    end
  end
  ...
  describe "#expiry=" do
    ...
    it "can be set in the initializer" do
      epoch = Time.now.to_i
      notification = EM::ApnManager::Notification.new(token, {}, {}, {:expiry => epoch})
      notification.expiry.should == epoch
    end
  end
  ...
  describe "#truncate_alert!" do
    context "when the data size would exceed the APN limit" do
      it "truncates the alert" do
        notification = EM::ApnManager::Notification.new(token, { "alert" => "X" * 300 })
        notification.data.size.should be > EM::ApnManager::Notification::DATA_MAX_BYTES

        notification.truncate_alert!
        notification.data.size.should_not be > EM::ApnManager::Notification::DATA_MAX_BYTES
      ...
      it "truncates the alert properly when it is JSON serialized into a different size" do
        notification = EM::ApnManager::Notification.new(token, { "alert" => '"' * 300 })
        notification.truncate_alert!
        parsed_payload = Yajl::Parser.parse(notification.payload)
      ...
      it "truncates the alert properly for multi-byte Unicode characters" do
        notification = EM::ApnManager::Notification.new(token, { "alert" => [57352].pack('U') * 500 })
        notification.truncate_alert!
        parsed_payload = Yajl::Parser.parse(notification.payload)
