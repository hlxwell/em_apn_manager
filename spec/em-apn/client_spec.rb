...
describe EventMachine::APN::Client do
  ...
  describe ".new" do
    it "creates a new client without a connection" do
      client = EM::ApnManager::Client.new
    ...
    context "configuring the gateway" do
      ...
      it "takes arguments for the gateway and port" do
        client = EM::ApnManager::Client.new(:gateway => "localhost", :port => 3333)
        client.gateway.should == "localhost"
    ...
    context "configuring SSL" do
      ...
      it "takes arguments for the key and cert" do
        client = EM::ApnManager::Client.new(:key => "key.pem", :cert => "cert.pem")
  ...
  describe "#connect" do
    it "creates a connection to the gateway" do
      ...
      client.connection.should be_an_instance_of(EM::ApnManager::Connection)
    end

    it "passes the client to the new connection" do
      client = EM::ApnManager::Client.new
      connection = double(EM::ApnManager::Connection).as_null_object

      EM::ApnManager::Connection.should_receive(:new).with(instance_of(Fixnum), client).and_return(connection)
      EM.run_block { client.connect }
    end
  end
  ...
  describe "#deliver" do
    ...
    it "logs a message" do
      ...
      EM::ApnManager.logger = Logger.new(test_log)
